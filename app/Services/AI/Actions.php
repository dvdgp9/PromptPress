        ...
        return [
            ...
            self::DESIGN_FORM => [
                ...
                'instruction'  =>
                  ...
                  . "  \"fields\": [ {\"label\":\"Etiqueta visible\",\"field_type\":\"text|email|tel|textarea|checkbox|select|number|date|url|file\",\"required\":true|false,\"options\":[\"Solo si field_type es select\"],\"file_accept\":\"documents|images|cv\",\"file_max_mb\":5} ],\n"
                  ...
                  . "}\n\n"
                  . "REGLAS:\n"
                  . "- Tipos de campo permitidos EXACTOS: text, email, tel, textarea, checkbox, select, number, date, url, file. Nada más.\n"
                  . "- Si usas select, añade 2-8 opciones claras en `options`. En otros tipos, omite `options` o devuélvelo vacío.\n"
                  . "- Si usas file, elige `file_accept`: documents, images o cv. Usa `file_max_mb` entre 1 y 10.\n"
            ...
            self::COMPOSE_CANVAS_PAGE => [
                ...
                'instruction'  =>
                  ...
                  . "- Colores y tipografías SOLO vía tokens del sitio: var(--pp-primary), var(--pp-primary-dark), var(--pp-accent), var(--pp-bg), var(--pp-surface), var(--pp-text), var(--pp-text-muted), var(--pp-on-primary), var(--pp-on-text), var(--pp-font-heading), var(--pp-font-body), var(--pp-radius-sm|md|lg|xl), var(--pp-shadow-sm|md|lg|xl), var(--pp-container-max).\n"
                  . "- DOS colores de marca, con dosis distintas: var(--pp-primary) manda (CTA principal, enlaces, énfasis) y var(--pp-accent) es el ACENTO SECUNDARIO, que acompaña con MUCHA menos presencia: detalles, badges, subrayados, iconos, numeración, bordes o un hover secundario. Úsalo en 1-3 sitios de toda la página, nunca en el CTA principal ni como fondo de una banda entera. Si la marca tiene un segundo color, tiene que RECONOCERSE en la página: no lo ignores.\n"
                  . "- Tonos derivados SIEMPRE con color-mix(in srgb, var(--pp-primary) N%, ...) o transparencias (también valen sobre var(--pp-accent)). PROHIBIDO inventar colores hex/rgb propios (única excepción: blanco/negro puros en overlays sobre fotografía).\n"
                  ...
                  . "CONTENIDO APORTADO (regla dura cuando exista 'CONTENIDO APORTADO POR EL USUARIO'):\n"
                  . "- Ese texto es la ÚNICA fuente de hechos de la página. Repártelo por las secciones según la estructura; puedes resumirlo, reescribirlo y darle forma para que encaje y luzca, pero NUNCA inventes información que pueda importar: nombres propios, precios, fechas, cifras, porcentajes, datos de contacto, clientes, premios o claims que no estén en ese texto ni en la memoria del sitio.\n"
                  . "- Si falta un dato para una sección, redáctala con lo que hay (más general) o redúcela; no rellenes con datos plausibles inventados ni placeholders entre corchetes.\n\n"
            ...
            self::EDIT_CANVAS_SECTION => [
                ...
                'instruction'  =>
                  ...
                  . "- `<pp-css>`: solo reglas nuevas o que SOBRESCRIBEN a las existentes (van después en la cascada). Reutiliza los nombres de clase existentes cuando modificas algo; usa tokens de marca (var(--pp-*)) para colores/tipos: var(--pp-primary) es el color principal y var(--pp-accent) el acento secundario, que se usa MENOS y en detalles (badges, subrayados, iconos, hovers).\n"
            ...
            self::PLAN_SITE_CHANGES => [
                ...
                'instruction'  =>
                  ...
                  . "- MEJOR ALTERNATIVA VIABLE PARA PÁGINAS QUE AÚN NO EXISTEN: si el usuario pide subpáginas, fichas o niveles nuevos pero TODO su contenido puede incorporarse con claridad dentro de una página relacionada que sí existe, propón `aplicar` sobre esa página existente. Convierte las supuestas subpáginas en secciones, tarjetas o acordeones homogéneos con anclas y CTAs. NO marques todo ese contenido como no_viable solo porque no existan URLs separadas. La instrucción debe explicar completa y literalmente qué secciones nuevas se añaden.\n"
                  . "- Separa CONTENIDO de NAVEGACIÓN: el contenido consolidado anterior es `aplicar`; solo las URLs independientes y los cambios del menú o jerarquía global quedan en un item `no_viable` aparte. En `reason` explica que se ofrece la versión equivalente dentro de la página existente.\n"
                  . "- No inventes IDs, slugs ni nombres de páginas inexistentes. Solo cita IDs que aparezcan literalmente en el MAPA DEL SITIO; para lo inexistente usa page_id 0 sin atribuirle ningún ID.\n"
