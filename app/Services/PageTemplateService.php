...
final class PageTemplateService
{
    ...
    /**
     * T18.8 — Sugiere hasta N plantillas para una página propuesta por la IA.
     * Heurística pura (sin llamada IA extra) para no inflar coste:
     *   1. Filtra plantillas cuyo `page_type` coincide.
     *   2. Si quedan menos de N, expande con tipos compatibles (home↔landing, service↔landing).
     *   3. Si aún quedan menos, añade fallbacks genéricos (home-default, service-pro).
     *   4. Reordena por score = matches de keywords del título/razón vs label+description.
     *
     * @param string $pageType  one of: home, landing, contact, service, about, article, generic
     * @param string $title
     * @param string $reason
     * @param int    $limit
     * @return string[] slugs ordenados por relevancia
     */
    public static function suggestForPage(string $pageType, string $title, string $reason = '', int $limit = 3): array
    {
        $all = self::all();
        if (empty($all)) return [];
        ...
        // Mapa de tipos compatibles (orden = preferencia).
        $expand = match ($pageType) {
            'home'    => ['home', 'landing'],
            'landing' => ['landing', 'home'],
            'service' => ['service', 'landing', 'home'],
            'contact' => ['contact'],
            'about'   => ['about', 'home', 'landing'],
            'article' => ['landing', 'home'],
            default   => ['landing', 'home'],
        };

        // Filtrar por tipos compatibles, conservando el orden de preferencia.
        $candidates = [];
        foreach ($expand as $t) {
            foreach ($all as $slug => $tpl) {
                if (($tpl['page_type'] ?? '') === $t && !isset($candidates[$slug])) {
                    $candidates[$slug] = $tpl;
                }
            }
        }

        // Fallback duro: si tras todo no hay nada, asegurar al menos home-default.
        if (empty($candidates) && isset($all['home-default'])) {
            $candidates['home-default'] = $all['home-default'];
        }

        // Score por keyword overlap.
        $needle = mb_strtolower(trim($title . ' ' . $reason));
        $needleWords = array_filter(preg_split('/\W+/u', $needle) ?: [], fn($w) => mb_strlen($w) >= 4);
        $needleWords = array_values(array_unique($needleWords));

        $scored = [];
        foreach ($candidates as $slug => $tpl) {
            $hay = mb_strtolower(($tpl['label'] ?? '') . ' ' . ($tpl['description'] ?? '') . ' ' . $slug);
            $score = 0;
        ...
        // Orden estable: score desc, luego orden de aparición original (preferencia por type-match).
        $i = 0;
        foreach ($scored as &$s) { $s['_i'] = $i++; }
        usort($scored, fn($a, $b) => ($b['score'] <=> $a['score']) ?: ($a['_i'] <=> $b['_i']));

        return array_slice(array_map(fn($s) => $s['slug'], $scored), 0, $limit);
    }

    /** Lista de tipos de sección que típicamente requieren imagen externa. */
    public static function imageHungrySections(array $tpl): array
    {
        $out = [];
        foreach ($tpl['sections'] ?? [] as $s) {
            $type = (string) ($s['type'] ?? '');
            if (in_array($type, ['hero', 'text_image', 'gallery', 'testimonials', 'logos_strip'], true)) {
                $out[] = $type;
            }
        }
