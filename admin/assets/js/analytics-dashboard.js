/**
 * analytics-dashboard.js — dashboard del módulo Analytics (FEAT-3 A5).
 *
 * Pinta la gráfica diaria (SVG generado a mano, sin librerías), los KPIs con
 * deltas vs. el periodo anterior y los desgloses con barras de proporción.
 * El selector de rango pide JSON a /admin/analytics/data y re-renderiza sin
...
(function () {
  ...
  function renderDevices(rows) {
    ...
    rows.forEach(function (row) {
      ...
      div.innerHTML =
        '<span class="pp-analytics-device__icon">' + (DEVICE_ICONS[row.k] || '💻') + '</span>' +
  ...
  function render(s) {
    ...
    breakdowns.style.display = hasData ? '' : 'none';
    chartWrap.parentElement.style.display = hasData ? '' : 'none';
  ...
  // ------------------------------------------------------- Range switching
  var loading = false;
  root.closest('.pp-main, body').querySelectorAll('.pp-analytics-range').forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      fetch(endpoint + '?range=' + btn.getAttribute('data-range'), { headers: { 'Accept': 'application/json' } })
