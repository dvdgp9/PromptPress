...
<section class="pp-inbox">
    ...
    <form class="pp-inbox-toolbar" method="GET" action="<?= e(base_url('admin/forms')) ?>" data-inbox-filters>
        ...
        <div class="pp-inbox-search">
            <label class="pp-visually-hidden" for="inbox-search">Buscar mensajes</label>
            <input id="inbox-search" type="search" name="q" value="<?= e((string) ($filters['q'] ?? '')) ?>" placeholder="Nombre, email, teléfono o contenido">
        ...
        <a class="pp-inbox-clear <?= $hasFilters ? '' : 'is-disabled' ?>" href="<?= e(base_url('admin/forms')) ?>" aria-disabled="<?= $hasFilters ? 'false' : 'true' ?>">Limpiar filtros</a>
        <label class="pp-inbox-filter"><span>Formulario</span><select name="form_id"><option value="0">Todos los formularios</option><?php foreach ($forms as $form): ?><option value="<?= (int) $form['id'] ?>" <?= (int) ($filters['form_id'] ?? 0) === (int) $form['id'] ? 'selected' : '' ?>><?= e((string) $form['heading']) ?></option><?php endforeach; ?></select></label>
        <label class="pp-inbox-filter"><span>Página</span><select name="page_id"><option value="0">Todas las páginas</option><?php foreach ($originPages as $origin): ?><option value="<?= (int) $origin['id'] ?>" <?= (int) ($filters['page_id'] ?? 0) === (int) $origin['id'] ? 'selected' : '' ?>><?= e((string) $origin['title']) ?></option><?php endforeach; ?></select></label>
        <label class="pp-inbox-filter"><span>Periodo</span><select name="period" data-inbox-period><option value="">Cualquier fecha</option><?php foreach ($periodLabels as $value => $label): ?><option value="<?= e((string) $value) ?>" <?= $effectivePeriod === (string) $value ? 'selected' : '' ?>><?= e($label) ?></option><?php endforeach; ?></select></label>
        <label class="pp-inbox-filter"><span>Entrega</span><select name="delivery"><option value="">Cualquier estado</option><?php foreach ($deliveryLabels as $value => $label): ?><option value="<?= e($value) ?>" <?= ($filters['delivery'] ?? '') === $value ? 'selected' : '' ?>><?= e($label) ?></option><?php endforeach; ?></select></label>
